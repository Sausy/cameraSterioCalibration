...
class GymStepRequest(genpy.Message):
  ...
  _full_text = """int8 controller #position, vel or displacement
float64[] set_points
float64 step_size
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.set_points))
      _x = self.step_size
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.set_points = s.unpack(str[start:end])
      start = end
      end += 8
...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.qdot))
      _x = self.feasible
      buff.write(_get_struct_B().pack(_x))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.qdot = s.unpack(str[start:end])
      start = end
      end += 1
